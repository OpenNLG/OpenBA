...
class _SentencePieceTokenizer(AbstractTokenizer):
    ...
    def _initalize(self, vocab_extra_ids):
        ...
        self._t5_sent_tokens = ['<pad>', '</s>', '<unk>']
        self._t5_mask_tokens = [f'<extra_id_{i}>' for i in range(vocab_extra_ids - 1, -1, -1)]
        self._t5_task_tokens = ['<R>', '<S>', '<X>']
        self._t5_tokens = self._t5_sent_tokens + self._t5_mask_tokens + self._t5_task_tokens
        ...
        # add sent special token
        for token in self._t5_tokens:
            _add_special_token(token)

        self._pad_id = self._vocab['<pad>']
        self._bos_id = self._vocab['<pad>']
        self._eos_id = self._vocab['</s>']
        self._unk_id = self._vocab['<unk>']
        self._rtask_id = self._vocab['<R>']
        self._stask_id = self._vocab['<S>']
        self._xtask_id = self._vocab['<X>']
    ...
    def detokenize(self, ids, skip_special_tokens=False):
        text = ""
        last_i = 0
    ...
    @property
    def rtask_id(self):
        return self._rtask_id

    @property
    def stask_id(self):
        return self._stask_id

    @property
    def xtask_id(self):
        return self._xtask_id

    @property
    def sentinel_tokens_ids(self):
        return [self.vocab[k] for k in self._t5_mask_tokens]
