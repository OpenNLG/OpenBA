...
def generate_tokens_probs_and_return_on_first_stage(
        model, encoder_inputs, tokens, lengths,
    ...
    with torch.no_grad():
        # attention_mask, position_ids = _build_attention_mask_and_position_ids(tokens)
        encoder_input_ids, tokens, encoder_attn_mask, \
                decoder_attn_mask, encoder_decoder_attn_mask = _build_t5_attention_mask_and_position_ids(encoder_inputs, tokens)

        encoder_hidden_states, _ =  model(encoder_input_ids, tokens, encoder_attn_mask, 
                                          decoder_attn_mask, encoder_decoder_attn_mask, output_hidden_states=True)

        prev_context_length = 0
        for context_length in range(min_prompt_length, max_sequence_length):
            # print(context_length)
            tokens2use = tokens[:, :context_length]
            decoder_attn_mask2use = decoder_attn_mask[..., :context_length, :context_length]
            encoder_decoder_attn_mask2use = encoder_decoder_attn_mask[..., :context_length, :]
            
            logits = forward_step(encoder_hidden_states, tokens2use, decoder_attn_mask2use, encoder_decoder_attn_mask2use)
...
def beam_search_and_return_on_first_stage(model, encoder_inputs, tokens, lengths, beam_size, stop_token, num_return_gen, length_penalty, prevent_newline_after_colon=True):
    args = get_args()
    tokenizer = get_tokenizer()
    ...
    # =============
    with torch.no_grad():
        encoder_inputs = encoder_inputs.repeat(beam_size, 1)
...
def make_history_mask(block):
    ...
    arange = torch.arange(length, device=torch.cuda.current_device())
    history_mask = (arange[None, None, :] <= arange[None, :, None])
    return history_mask


def _build_t5_attention_mask_and_position_ids(encoder_inputs, decoder_inputs):
    encoder_input_ids = encoder_inputs
    decoder_input_ids = decoder_inputs
    # encoder_attn_mask = 
    # tokens_enc = train_sample['text_enc']
    # tokens_dec_in = train_sample['text_dec']
    encoder_attn_mask = (encoder_inputs[:, None, :] >= 1) * (encoder_inputs[:, :, None] >= 1)
    encoder_decoder_attn_mask = (encoder_inputs[:, None, :] >= 1) * (decoder_inputs[:, :, None] >= 0)
    # decoder_attn_mask = make_attention_mask(decoder_inputs, decoder_inputs)
    decoder_attn_mask = make_history_mask(decoder_inputs)
    encoder_attn_mask = encoder_attn_mask < 0.5
    decoder_attn_mask = decoder_attn_mask < 0.5
    encoder_decoder_attn_mask = encoder_decoder_attn_mask < 0.5

    return encoder_input_ids, decoder_input_ids, encoder_attn_mask, \
