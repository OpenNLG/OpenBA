...
def generate_and_post_process(model,
    ...
    if mpu.is_pipeline_first_stage():
        tokens, prompts_plus_generations = detokenize_generations(tokens, lengths, False)
...
def generate(model,
    ...
    values = [tokens_to_generate,
              ...
              random_seed, encoder_seq_length]
    ...
    encoder_seq_length = int(values_float_tensor[13].item())
    # print(top_p_sampling, temperature)
    if random_seed != -1:
    ...
    encoder_context_tokens_tensor, context_tokens_tensor, context_length_tensor = tokenize_prompts(
        encoder_inputs=encoder_inputs, prompts=prompts, 
        tokens_to_generate=tokens_to_generate, encoder_seq_length=encoder_seq_length)
    ...
    return generate_tokens_probs_and_return_on_first_stage(
        model, encoder_context_tokens_tensor, context_tokens_tensor, context_length_tensor,
        return_output_log_probs=return_output_log_probs,
...
def beam_search_and_post_process(model,
    ...
    if mpu.is_pipeline_first_stage():
        ...
        return prompts_plus_generations, scores

    return None

def beam_search(model, encoder_inputs=None, encoder_seq_length=None, prompts=None, tokens_to_generate=0, beam_size=0, add_BOS=False, stop_token=50256, num_return_gen=1, length_penalty=1, prevent_newline_after_colon=False):
    # Make sure input params are avaialble to all ranks.
    values = [tokens_to_generate,
              beam_size,
              add_BOS,
    ...
    encoder_inputs_token_tensor, context_tokens_tensor, context_length_tensor = tokenize_prompts(
        encoder_inputs=encoder_inputs, encoder_seq_length=encoder_seq_length,
        prompts=prompts, tokens_to_generate=tokens_to_generate)

    return beam_search_and_return_on_first_stage(model, encoder_inputs_token_tensor, context_tokens_tensor, context_length_tensor, 
