...
class ForwardStep:
    ...
    def __call__(self, encoder_hidden_states, tokens, decoder_attn_mask, encoder_decoder_attn_mask):
...
def _forward_step_helper(model, encoder_hidden_states, tokens, decoder_attn_mask, encoder_decoder_attn_mask,
    ...
    output_tensor = model(tokens, tokens, decoder_attn_mask, decoder_attn_mask, encoder_decoder_attn_mask, 
                          enc_hidden_states=encoder_hidden_states, inference_params=inference_params)
...
def _no_pipelining_forward_step(model, encoder_hidden_states, tokens, decoder_attn_mask, encoder_decoder_attn_mask,
                                inference_params, recv_buffer=None):
    """If recv_buffer is none, we will allocate one on the fly."""
    # Run a simple forward pass.
    output_tensor = _forward_step_helper(model, encoder_hidden_states, tokens, decoder_attn_mask,
...
def _with_pipelining_forward_step(model, encoder_hidden_states, tokens, position_ids, attention_mask,
    ...
    for micro_batch_index in range(num_micro_batches):
        ...
        if this_micro_batch_size != micro_batch_size:
            recv_buffer = None
        output = _forward_step_helper(model, encoder_hidden_states, tokens2use, position_ids2use,
