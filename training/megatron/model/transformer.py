...
from megatron.model.rotary_embedding_torch import (
    RotaryEmbedding,
...
class SwiGLUMLP(MegatronModule):
    """SwiGLU activation
    """
    def __init__(self, init_method, output_layer_init_method):
        ...
        ffn_hidden_size = int(2 * args.ffn_hidden_size / 3) 
    ...
    def forward(self, hidden_states):
        # hidden_states [s, b, 4hp]
        # import rpdb; rpdb.set_trace('0.0.0.0', 892+torch.distributed.get_rank())
        hidden_states_1, _  = self.w1(hidden_states)
        hidden_states_1 = F.silu(hidden_states_1)
        hidden_states_3, _ = self.w3(hidden_states)
        hidden_states_3 = hidden_states_3 * hidden_states_1
        return self.w2(hidden_states_3)
...
class ParallelAttention(MegatronModule):
    ...
    def __init__(self, init_method,
                 output_layer_init_method, layer_number, use_rotary,
        ...
        if attention_type != AttnType.cross_attn and use_rotary:
            self.rotary_embed = RotaryEmbedding(args.kv_channels, precision=self.params_dtype)
    ...
    def forward(self, hidden_states, attention_mask,
        ...
        if self.rotary_embed:
            # q, k = [rearrange(x, 's b h d -> b h s d').contiguous()  # [b, h, s, d]
            #             for x in (query_layer, key_layer)]
            # q = self.rotary_embed.rotate_queries_or_keys(q)
            # k = self.rotary_embed.rotate_queries_or_keys(k)

            # q, k = [rearrange(x, 'b h s d -> s b h d') for x in (q,k)]  # [b, s, h, d]
            apply_rotary_fn = apply_rotary_pos_emb
            seq_len = key_layer.shape[0]
            cos, sin = self.rotary_embed(value_layer, seq_len=seq_len)
            q, k = apply_rotary_fn(
                query_layer, key_layer, cos, sin
            )
            context_layer = self.core_attention(q, k, value_layer, attention_mask)
...
def no_bias_dropout_add(x, residual, prob, training):
...
@torch.jit.script
def no_bias_dropout_add_fused_train(x: torch.Tensor,
                                 residual: torch.Tensor,
                                 prob: float) -> torch.Tensor:
    return no_bias_dropout_add(x, residual, prob, True)
...
@torch.jit.script
def no_bias_dropout_add_fused_inference(x: torch.Tensor,
                                        residual: torch.Tensor,
                                        prob: float) -> torch.Tensor:
    return no_bias_dropout_add(x, residual, prob, False)
...
class ParallelTransformerLayer(MegatronModule):
    ...
    def __init__(self, init_method, output_layer_init_method,
                 layer_number, pos_emb_type, layer_type=LayerType.encoder,
        ...
        self.self_attention = ParallelAttention(
            ...
            layer_number,
            use_rotary=(pos_emb_type == "rotary"),
            attention_type=AttnType.self_attn,
        ...
        elif args.mlp_type == "origin":
            self.mlp = ParallelMLP(init_method, output_layer_init_method)
        elif args.mlp_type == "SwiGLU":
            self.mlp = SwiGLUMLP(init_method, output_layer_init_method)
    ...
    def forward(self, hidden_states, attention_mask,
        ...
        if self.drop_path is None:
            ...
            if self.bias_dropout_fusion:
                if self.training:
                    ...
                    no_bias_dropout_add_func = no_bias_dropout_add_fused_train
                else:
                    bias_dropout_add_func = bias_dropout_add_fused_inference
                    no_bias_dropout_add_func = no_bias_dropout_add_fused_inference
            else:
        ...
        if self.drop_path is None:
            with self.bias_dropout_add_exec_handler():
                if mlp_bias is None:
                    output = no_bias_dropout_add_func(
                    mlp_output,
                    residual,
                    self.hidden_dropout)
                else:
            ...
            out = torch.nn.functional.dropout(mlp_output + mlp_bias if mlp_bias is not None else mlp_output,
                                              p=self.hidden_dropout,
                                              training=self.training)
...
class ParallelTransformer(MegatronModule):
    ...
    def __init__(self, init_method, output_layer_init_method, pos_emb_type,
        ...
        def build_layer(layer_number):
            if args.transformer_impl == 'local':
                return ParallelTransformerLayer(
                    init_method,
                    output_layer_init_method,
                    layer_number, pos_emb_type=pos_emb_type,
